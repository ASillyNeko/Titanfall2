	...
	for ( int row = 0; row < MAX_COMBO_BUTTON_ROWS; row++ )
	{
		...
		for ( int col = 0; col < MAX_COMBO_BUTTON_COLS; col++ )
		{
			...
			if ( col == 0 )
			{
				...
				Hud_SetYOverTime( button, yOffset, TRANSITION_ANIM_TIME, INTERPOLATOR_DEACCEL )
			}
			...
			if ( rowIndex == row )
			{
				button.SetHeight( comboStruct.expandedHeight )
				foreach ( var childElement in button.s.alwaysChildren )
				{
			...
			{
				button.SetHeight( comboStruct.collapsedHeight )
				foreach ( var childElement in button.s.alwaysChildren )
				{
