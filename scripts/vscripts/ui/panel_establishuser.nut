...
void function InitEstablishUserPanel()
{
	RegisterSignal( "EndShowEstablishUserPanel" )

	file.panel = GetPanel( "EstablishUserPanel" )
...
void function OnShowEstablishUserPanel()
{
	...
	while ( Hud_IsVisible( file.panel ) )
	{
		...
		{
			if ( lastState == -1 )
				printt( sig + "userSignInState changed from: -1 to:", Dev_GetEnumString( userSignInState, state ) )
			else
				printt( sig + "userSignInState changed from:", Dev_GetEnumString( userSignInState, lastState ), "to:", Dev_GetEnumString( userSignInState, state ) )
			...
			{
				if ( IsDialog( uiGlobal.activeMenu ) ) // Close data center dialog if it's open
...
void function OnHideEstablishUserPanel()
{
	Signal( uiGlobal.signalDummy, "EndShowEstablishUserPanel" )
}
